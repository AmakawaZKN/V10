...
const expiredCheck = (lexxy, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            lexxy.sendMessage(_dir[position].id, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar! Dadahh ><` })
            .then( res => {
              lexxy.groupLeave(_dir[position].id)
